...
const DEFAULT_FRONTEND_URL = 'https://www.tryinterview.site';
...
async function getUsageSummary(firebaseUid) {
  ...
  const rows = await query(`
    SELECT
      ...
      COUNT(*) FILTER (WHERE created_at >= $2)::integer AS month_interviews,
      COUNT(*) FILTER (WHERE status = 'completed')::integer AS completed_interviews
    FROM interviews i
...
async function saveUserProfile(firebaseUid, payload = {}, identity = {}) {
  ...
  } else if (identity.photoURL && !currentUser?.photoURL) {
